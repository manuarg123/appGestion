...
export class SpecialtyFormComponent {
  ...
  constructor(
    private dialogRef: MatDialogRef<SpecialtyFormComponent>,
  ...
  handleSubmit(): void {
    ...
    if (token != null) {
      if (id == "") {
        this.apiService.post('specialities', token, data).subscribe(
          (response) => {
            this.specialtyDataService.triggerSpecialtyAdded();
        ...
        this.apiService.put('specialities', token, data, id).subscribe(
          (response) => {
            this.specialtyDataService.triggerSpecialtyAdded();
            this.dialogRef.close();
          },
